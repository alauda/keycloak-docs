...
In Kubernetes, Keycloak uses the KUBE_PING or DNS_PING JGroups protocol for cluster discovery. The Operator configures this automatically. Ensure that:

- Keycloak Pods can communicate on the JGroups port (default: `7800`).
- Network policies do not block inter-Pod traffic on this port.
- The Keycloak service account has permission to list Pods in the namespace (required for KUBE_PING).
